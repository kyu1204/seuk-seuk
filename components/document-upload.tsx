...
export default function DocumentUpload() {
  ...
  const [showSignatureRequestDialog, setShowSignatureRequestDialog] =
  ...
  const handleAreaSelected = async (area: {
    ...
    const newAreas = [...signatureAreas, newArea];
    ...
    console.log(
      '📝 [UI] 서명 영역을 로컬에 임시 저장했습니다. "저장하기" 버튼을 누르면 데이터베이스에 저장됩니다.'
    );
  };
  ...
  const handleRemoveArea = async (index: number) => {
    ...
    setSignatureAreas(newAreas);
    console.log(
      '🗑️ [UI] 서명 영역을 로컬에서 제거했습니다. "저장하기" 버튼을 누르면 데이터베이스에 반영됩니다.'
    );
  };
  ...
  const handleSaveSignatureAreas = async () => {
    ...
    try {
      const result = await saveSignatureAreas(
        currentDocument.id,
        signatureAreas
      );
              ...
              <CardContent className="p-6 space-y-4">
                ...
                <div className="flex gap-4">
                  <Button
                    ...
                    className={`${
                      isUploading || !documentTitle.trim()
            ...
            {isSelecting ? (
              ...
              <div ref={documentContainerRef} className="relative">
                ...
                {signatureAreas.map((area, index) => {
                  ...
                  if (
                    documentImageRef.current &&
                    documentImageRef.current.complete
                  ) {
                    const containerRect =
